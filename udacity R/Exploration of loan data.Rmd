...
dt <- tidyr::separate(dt,LoanOriginationDate, c("LoanOriginationYear","LoanOriginationMonth"),sep="-",extra="drop")
ggplot(data=dt,aes(x=LoanOriginationYear)) + geom_histogram()
```
...
ggplot(data=dt,aes(x=ProsperScore)) + 
  geom_histogram()

ggplot(data=dt,aes(x=ProsperRating..numeric.)) + 
  geom_histogram()
```
...
ggplot(data=dt,aes(x=ListingCategory..numeric.)) + 
  geom_histogram(binwidth=1) + scale_x_continuous(breaks=seq(0,20,1))
...
ggplot(data=dt,aes(x=CreditScoreRangeLower)) + 
  geom_histogram(binwidth=10) + 
  scale_x_continuous(breaks=seq(500,max(dt$CreditScoreRangeLower, na.rm=T),10)) + 
  coord_cartesian(xlim = c(500,max(dt$CreditScoreRangeLower,na.rm=T)))

ggplot(data=dt,aes(x=CreditScoreRangeUpper)) + 
  geom_histogram(binwidth=10) + 
  scale_x_continuous(breaks=seq(500,max(dt$CreditScoreRangeUpper, na.rm=T),10)) + 
  coord_cartesian(xlim = c(500,max(dt$CreditScoreRangeUpper,na.rm=T)))
```
...
ggplot(data=dt,aes(x=DebtToIncomeRatio)) + geom_histogram(binwidth=.05) +
...
dt$IncomeRange <- factor(dt$IncomeRange,
                         c("Not displayed","Not employed",
                          ...
                          "$100,000+"))
ggplot(data=dt,aes(IncomeRange)) + geom_bar()
```
...
ggplot(dt,aes(StatedMonthlyIncome)) + geom_histogram(binwidth=500) + coord_trans(limx = c(0,quantile(dt$StatedMonthlyIncome,.99)))
```
...
str(dt$Occupation)
?coord_flip()
ggplot(data=dt,aes(Occupation)) + geom_bar() + coord_flip()
```
...
ggplot(dt,aes(EmploymentStatus)) + geom_bar() + coord_flip()
```
...
ggplot(dt,aes(AmountDelinquent)) + geom_histogram() + coord_trans(limx = c(0,))

summary(dt$DelinquenciesLast7Years)
ggplot(dt,aes(DelinquenciesLast7Years)) + geom_histogram(binwidth=1) + coord_trans(limx = c(0,50), limy=c(0,5000)) + scale_x_continuous(breaks=seq(0,50,5)) + scale_y_continuous(breaks=seq(0,5000,500))
```
...
ggplot(dt,aes(LoanOriginalAmount)) + geom_histogram(binwidth=1000) + scale_x_continuous(breaks=seq(1000,30000,1000), limits=c(1000,30000))

ggplot(dt,aes(LoanOriginalAmount)) + geom_histogram(binwidth=100) + scale_x_continuous(breaks=seq(1000,10000,100), limits=c(1000,10000)) + coord_flip()
...
ggplot(dt,aes(Term)) + geom_histogram() + scale_x_continuous(breaks=c(12,36,60))
...
```{r}
str(dt$BorrowerState)
ggplot(dt,aes(BorrowerState)) + geom_bar() + coord_flip()
```
...
```{r}
str(dt$ClosedDate)
dt <- tidyr::separate(dt,ClosedDate, c("ClosedYear","ClosedMonth"),sep="-",extra="drop")
ggplot(dt,aes(ClosedYear)) + geom_histogram()
```
...
```{r}
str(dt$LoanStatus)
ggplot(dt,aes(LoanStatus)) + geom_bar() + coord_flip()
```
...
ggplot(subset(dt,dt$LP_NetPrincipalLoss > 0),aes(LP_NetPrincipalLoss)) + geom_histogram(binwidth=1000) + scale_x_continuous(breaks=seq(0,25000,1000))
```
...
```{r}
?transform
dt <- transform(dt, LP_CustomerPayments_pc = LP_CustomerPayments / LoanOriginalAmount)

dt <- transform(dt, LP_CustomerPrincipalPayments_pc = LP_CustomerPrincipalPayments / LoanOriginalAmount)

dt <- transform(dt, LP_InterestandFees_pc = LP_InterestandFees / LoanOriginalAmount)

dt <- transform(dt, LP_ServiceFees_pc = LP_ServiceFees / LoanOriginalAmount)

dt <- transform(dt, LP_CollectionFees_pc = LP_CollectionFees / LoanOriginalAmount)

dt <- transform(dt, LP_GrossPrincipalLoss_pc = LP_GrossPrincipalLoss / LoanOriginalAmount)

dt <- transform(dt, LP_NetPrincipalLoss_pc = LP_NetPrincipalLoss / LoanOriginalAmount)

dt <- transform(dt, LP_NonPrincipalRecoverypayments_pc = LP_NonPrincipalRecoverypayments / LoanOriginalAmount)

#order BorrowerState by the number of observations
rec_per_state <- table(dt$BorrowerState)
rec_per_state <- sort(rec_per_state, decreasing = T)
names(rec_per_state)

dt$BorrowerState <- ordered(dt$BorrowerState,levels = names(rec_per_state))
...
str(dt$ClosedYear)
dtd <- subset(dt,!is.na(dt$ClosedYear) & dt$ClosedYear != "")
rec_per_state <- table(dtd$BorrowerState)
rec_per_state <- sort(rec_per_state)
dtd$BorrowerState <- ordered(dtd$BorrowerState,levels = names(rec_per_state))
```
...
```{r}
#create dataframe for when these columns are not null there are no empty strings, but just include this check for completeness
dtc <- subset(dtd,!is.na(dtd$ProsperRating..numeric. & dtd$ProsperRating..numeric. != ""))
rec_per_state <- table(dtc$BorrowerState)
rec_per_state <- sort(rec_per_state)
dtc$BorrowerState <- ordered(dtc$BorrowerState,levels = names(rec_per_state))
...
ggplot(dtd,aes(x=LP_CustomerPayments_pc)) + geom_histogram(binwidth=0.01) + scale_x_continuous(breaks=seq(0,2,.1), lim=c(0,2))
```

PrincipalPayments and Interest and Fees
Principal payments are either a full 100% of the original loan or loser to 10% or less of the original loan.  Interest and Fees are mostly 10% or less (the distribution is skewed right)
```{r}
names(dt)
summary(dtd$LP_CustomerPrincipalPayments_pc)
ggplot(dtd,aes(x=LP_CustomerPrincipalPayments_pc)) + geom_histogram(binwidth=0.01) + scale_x_continuous(breaks=seq(0,1.1,.1), lim=c(0,1.1))

summary(dtd$LP_InterestandFees_pc)
ggplot(dtd,aes(x=LP_InterestandFees_pc)) + geom_histogram(binwidth=0.01) + scale_x_continuous(breaks=seq(0,1.1,.1), lim=c(0,1.1))
```
...
```{r}
ggplot(dtd, aes(x = LP_NetPrincipalLoss_pc)) + geom_histogram()
```

BorrowerState
I ordred states by numer of observations per state.  California has twice as many observations as the second state, Texas.  About 5,500 observations do not have a state specified.
```{r}
ggplot(dt,aes(BorrowerState)) + geom_histogram()

table(dt$BorrowerState)
```
...
###Payment amounts
These relate to how much the borrower paid as fraction of the original loan.
...
ggplot(dtc,aes(x=LP_CustomerPayments_pc, y=ListingNumber,color=ProsperRating..numeric.)) + geom_point(alpha=0.1)

ggplot(dt,aes(y=LP_CustomerPayments_pc,x=factor(ProsperRating..numeric.))) + geom_boxplot() + ggtitle("Including in progress loans makes payments misleading")

ggplot(dtc,aes(y=LP_CustomerPayments_pc,x=factor(ProsperRating..numeric.))) + geom_boxplot() + ggtitle("Including only completed loans makes comparisons more clear")

ggplot(dtc,aes(y=LP_CustomerPayments_pc,x=factor(ProsperScore))) + geom_boxplot() + ggtitle("ProsperScore should only range from 1 to 10, why is there 11?")

ggplot(dtc,aes(x=LP_CustomerPayments_pc)) + facet_wrap(~ProsperRating..numeric.,ncol=7) + geom_histogram() + ggtitle("Customer payments faceted by Prosper Rating")
...
```{r}
ggplot(dtc,aes(y=LP_InterestandFees_pc,x=factor(ProsperRating..numeric.))) + geom_boxplot() + ggtitle("Lower scored borrowers paid more interest and fees")
...
```{r}
ggplot(dtc,aes(y=LP_CustomerPrincipalPayments_pc,x=factor(ProsperRating..numeric.))) + geom_boxplot() + ggtitle("Lower scored borrowers paid less of the principal")
...
ggplot(dtc,aes(y=LP_NonPrincipalRecoverypayments_pc,x=factor(ProsperRating..numeric.))) + geom_boxplot() + coord_cartesian(ylim = c(0,0.1))+ ggtitle("Most borrowers do not pay recovery payments")

summary(subset(dtc,dtc$LP_NonPrincipalRecoverypayments_pc>0)$LP_NonPrincipalRecoverypayments_pc)

ggplot(subset(dtc,dtc$LP_NonPrincipalRecoverypayments_pc>0),aes(y=LP_NonPrincipalRecoverypayments_pc,x=factor(ProsperRating..numeric.))) + geom_boxplot() + ggtitle("borrowers who made recovery payments paid more when they had lower ratings")
```

LP_GrossPrincipalLoss_pc
Corr is -0.24 because lower rated borrowers had larger losses
```{r}
ggplot(dtc,aes(y=LP_GrossPrincipalLoss_pc,x=factor(ProsperRating..numeric.))) + geom_boxplot() + ggtitle("Gross Principal Losses are higher for lower rated borrowers")
...
```{r}
ggplot(dtc,aes(y=LP_NetPrincipalLoss_pc,x=factor(ProsperRating..numeric.))) + geom_boxplot() + ggtitle("Net Principal losses are higher for lower rated borrowers")
...
ggplot(dtd,aes(x=StatedMonthlyIncome,y=LP_CustomerPayments_pc)) + geom_point(alpha=.2) + coord_cartesian(xlim = c(0,quantile(dtd$StatedMonthlyIncome,probs = .99)))
?cut
quantile(dtd$StatedMonthlyIncome,seq(0,1,.25))
quantile(dtd$StatedMonthlyIncome,seq(0,1,.1),)

summary(dtd$StatedMonthlyIncome)
dtd <- transform(dtd, StatedMonthlyIncomeRange = cut(dtd$StatedMonthlyIncome, breaks=quantile(dtd$StatedMonthlyIncome,seq(0,1,.1)),include.lowest = TRUE ))

summary(dtd$StatedMonthlyIncomeRange)
ggplot(dtd,aes(x=StatedMonthlyIncomeRange,y=LP_CustomerPayments_pc)) + geom_boxplot() + coord_flip() + ggtitle("Median payments are similar across income, with more variability in lower income")
...
ggplot(dtd,aes(x=DebtToIncomeRatio,y=LP_CustomerPayments_pc)) + geom_point(alpha=0.1)
...
dtd <- transform(dtd,DebtToIncomeRatioRange = cut(dtd$DebtToIncomeRatio, breaks= quantile(dtd$DebtToIncomeRatio,seq(0,1,.1),na.rm=T), include.lowest = T))

summary(dtd$DebtToIncomeRatioRange)
ggplot(subset(dtd,!is.na(dtd$DebtToIncomeRatio)),aes(x=DebtToIncomeRatioRange,y=LP_CustomerPayments_pc)) + geom_boxplot() + ggtitle("Higher debt to income ratios had lower payments")
...
ggplot(dtd,aes(x=CreditScoreRangeLower,y=LP_CustomerPayments_pc)) + geom_point(alpha=0.1,fill="grey",position="jitter")
...
ggplot(subset(dtd,!is.na(dtd$CreditScoreRangeLower)),aes(x=CreditScoreRangeLowerRange,y=LP_CustomerPayments_pc)) + geom_boxplot() + ggtitle("lower credit scores had slightly higher median payments, but more variance")
...
```{r}
?geom_point
ggplot(dtd,aes(x=LoanOriginalAmount,y=LP_CustomerPayments_pc)) + geom_jitter(alpha=0.1,color="grey")

dtd <- transform(dtd,LoanOriginalAmountRange = cut(dtd$LoanOriginalAmount, breaks=quantile(dtd$LoanOriginalAmount,seq(0,1,.1),na.rm=T),include.lowest = T))

table(dtd$LoanOriginalAmountRange)

ggplot(dtd,aes(x=LoanOriginalAmountRange,y=LP_CustomerPayments_pc)) + geom_boxplot() + coord_flip() + ggtitle("Those that borrowed the most had more risk of under-paying")

ggplot(dtd,aes(x=StatedMonthlyIncomeRange,y=LoanOriginalAmount)) + geom_boxplot() + coord_flip()

ggplot(subset(dtd,dtd$StatedMonthlyIncome <= quantile(dtd$StatedMonthlyIncome,.99)),aes(y=StatedMonthlyIncome,x=LoanOriginalAmountRange)) + geom_boxplot() + coord_flip()
...
```{r}
dtd <- transform(dtd, LoanOriginalAmount_per_Term = LoanOriginalAmount / Term)
dtd <- transform(dtd, LoanOriginalAmount_per_TermRange = cut(dtd$LoanOriginalAmount_per_Term, breaks = quantile(dtd$LoanOriginalAmount_per_Term, probs = seq(0,1,.1), na.rm = T),include.lowest = T))

ggplot(dtd,aes(x=LoanOriginalAmount_per_TermRange,y=LP_CustomerPayments_pc)) + geom_boxplot()
```
...
dtd <- transform(dtd, LoanOriginalAmount_per_Term_pc_Income  = 100*LoanOriginalAmount_per_Term / StatedMonthlyIncome)

dtd <- transform(dtd, LoanOriginalAmount_per_Term_pc_IncomeRange = cut(dtd$LoanOriginalAmount_per_Term_pc_Income,breaks = quantile(dtd$LoanOriginalAmount_per_Term_pc_Income,probs = seq(0,1,.1),na.rm = T),include.lowest = T))

ggplot(dtd,aes(x=LoanOriginalAmount_per_Term_pc_IncomeRange,y=LP_CustomerPayments_pc)) + geom_boxplot() + coord_flip() + ggtitle("Loan per month as % of monthly income")
```
...
ggplot(dtd,aes(BorrowerState)) + geom_histogram() + scale_y_continuous(breaks=seq(0,8000,500)) + coord_flip()

ggplot(dtd,aes(x=BorrowerState,y=LP_CustomerPayments_pc)) + geom_boxplot() + coord_flip()
```
...
StatedMonthlyIncome, Prosper Rating, CreditScoreRangeLower, BorrowerState, and Customer Payments
...
inc_range <- sort(table(dtc$StatedMonthlyIncomeRange))
dtc$StatedMonthlyIncomeRange <- ordered(dtc$StatedMonthlyIncomeRange,names(inc_range))
summary(dtc$StatedMonthlyIncomeRange)

#round to the nearest $500 to get buckets for income
dtc <- transform(dtc, StatedMonthlyIncomeRangeNumeric = floor(StatedMonthlyIncome / 500)*500)
#?select
#dtc <- select(dtc, -one_of(c("StatedMonthlyIncomeRangeNumeric")) )

#ggplot(dtc,aes(x=StatedMonthlyIncome,y=LP_CustomerPayments_pc,color=factor(ProsperRating..numeric.))) + geom_line(stat='summary',fun.y=median) + scale_color_brewer(type="div",palette="Spectral") + coord_cartesian(xlim = c(0,quantile(dtc$StatedMonthlyIncome,probs = .99))) 

ggplot(dtc,aes(x=StatedMonthlyIncomeRangeNumeric,y=LP_CustomerPayments_pc,color=factor(dtc$ProsperRating..numeric.))) + geom_line(stat='summary',fun.y=median) + scale_color_brewer(type="seq",palette="RdYlBu") + coord_cartesian(xlim = c(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,probs = .99))) + scale_x_continuous(breaks=seq(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,.99),1000))

#?scale_color_brewer
#display.brewer.all(5)
#ggplot(dtc,aes(x=StatedMonthlyIncomeRangeNumeric,y=LP_CustomerPayments_pc,color=factor(ProsperRating..numeric.))) + geom_point(stat='summary',fun.y=median) + scale_color_brewer(type = "seq",palette="RdYlBu") + coord_cartesian(xlim = c(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,probs = .99)))
```
...
p_median <- ggplot(dtc,aes(x=StatedMonthlyIncomeRangeNumeric,y=LP_CustomerPayments_pc,color=factor(dtc$ProsperRating..numeric.))) + geom_line(stat='summary',fun.y=median) + scale_color_brewer(type="seq",palette="RdYlBu") + coord_cartesian(xlim = c(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,probs = .99))) + scale_x_continuous(breaks=seq(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,.99),1000)) + ggtitle("Median Customer Payments as % of loan")

p_25 <- ggplot(dtc,aes(x=StatedMonthlyIncomeRangeNumeric,y=LP_CustomerPayments_pc,color=factor(dtc$ProsperRating..numeric.))) + geom_line(stat='summary',fun.y=quantile,probs=.25) + scale_color_brewer(type="seq",palette="RdYlBu") + coord_cartesian(xlim = c(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,probs = .99))) + scale_x_continuous(breaks=seq(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,.99),1000)) + ggtitle("25% quantile of Customer Payments as % of loan")

p_75 <- ggplot(dtc,aes(x=StatedMonthlyIncomeRangeNumeric,y=LP_CustomerPayments_pc,color=factor(dtc$ProsperRating..numeric.))) + geom_line(stat='summary',fun.y=quantile,probs=.75) + scale_color_brewer(type="seq",palette="RdYlBu") + coord_cartesian(xlim = c(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,probs = .99))) + scale_x_continuous(breaks=seq(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,.99),1000)) + ggtitle("75% quantile of Customer Payments as % of loan")


#library(gridExtra)
grid.arrange(p_75,p_median,p_25,ncol=1)
```
...
p_median <- ggplot(dtc,aes(x=StatedMonthlyIncomeRangeNumeric,y=LP_InterestandFees_pc,color=factor(dtc$ProsperRating..numeric.))) + geom_line(stat='summary',fun.y=median) + scale_color_brewer(type="seq",palette="RdYlBu") + coord_cartesian(xlim = c(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,probs = .99))) + scale_x_continuous(breaks=seq(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,.99),1000)) + ggtitle("Median Interest and Fees as % of loan")

p_25 <- ggplot(dtc,aes(x=StatedMonthlyIncomeRangeNumeric,y=LP_InterestandFees_pc,color=factor(dtc$ProsperRating..numeric.))) + geom_line(stat='summary',fun.y=quantile,probs=.25) + scale_color_brewer(type="seq",palette="RdYlBu") + coord_cartesian(xlim = c(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,probs = .99))) + scale_x_continuous(breaks=seq(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,.99),1000)) + ggtitle("25% quantile of Interest and Fees as % of loan")

p_75 <- ggplot(dtc,aes(x=StatedMonthlyIncomeRangeNumeric,y=LP_InterestandFees_pc,color=factor(dtc$ProsperRating..numeric.))) + geom_line(stat='summary',fun.y=quantile,probs=.75) + scale_color_brewer(type="seq",palette="RdYlBu") + coord_cartesian(xlim = c(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,probs = .99))) + scale_x_continuous(breaks=seq(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,.99),1000)) + ggtitle("75% quantile of Interest and Fees as % of loan")
...
p_median <- ggplot(dtc,aes(x=StatedMonthlyIncomeRangeNumeric,y=LP_CustomerPrincipalPayments_pc,color=factor(dtc$ProsperRating..numeric.))) + geom_line(stat='summary',fun.y=median) + scale_color_brewer(type="seq",palette="RdYlBu") + coord_cartesian(xlim = c(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,probs = .99))) + scale_x_continuous(breaks=seq(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,.99),1000)) + ggtitle("Median Principal Payments as % of loan")

p_25 <- ggplot(dtc,aes(x=StatedMonthlyIncomeRangeNumeric,y=LP_CustomerPrincipalPayments_pc,color=factor(dtc$ProsperRating..numeric.))) + geom_line(stat='summary',fun.y=quantile,probs=.25) + scale_color_brewer(type="seq",palette="RdYlBu") + coord_cartesian(xlim = c(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,probs = .99))) + scale_x_continuous(breaks=seq(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,.99),1000)) + ggtitle("25% quantile of Principal Payments as % of loan")

p_75 <- ggplot(dtc,aes(x=StatedMonthlyIncomeRangeNumeric,y=LP_CustomerPrincipalPayments_pc,color=factor(dtc$ProsperRating..numeric.))) + geom_line(stat='summary',fun.y=quantile,probs=.75) + scale_color_brewer(type="seq",palette="RdYlBu") + coord_cartesian(xlim = c(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,probs = .99))) + scale_x_continuous(breaks=seq(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,.99),1000)) + ggtitle("75% quantile of Principal Payments as % of loan")
...
#   12    36    60 
# 1531 20778  3696 
p_obs <- ggplot(dtc,aes(x=StatedMonthlyIncomeRangeNumeric)) + geom_histogram(binwidth = 500) + coord_cartesian(xlim = c(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,probs = .99))) + scale_x_continuous(breaks=seq(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,.99),1000)) + facet_wrap(~Term) + ggtitle("Number of observations by income") + scale_color_brewer(type = 'div', guide = guide_legend(title = 'Count', reverse = F,override.aes = list(alpha = 1, size = 2)))

p_median <- ggplot(dtc,aes(x=StatedMonthlyIncomeRangeNumeric,y=LP_CustomerPayments_pc,color=factor(dtc$ProsperRating..numeric.))) + geom_line(stat='summary',fun.y=median) + scale_color_brewer(type="seq",palette="RdYlBu", guide = guide_legend(title='Rating', reverse = T, override.aes = list(alpha=1,size=2))) + coord_cartesian(xlim = c(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,probs = .99))) + scale_x_continuous(breaks=seq(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,.99),1000)) + facet_wrap(~Term) + ggtitle("Median Customer Payments as % of loan, facet by term (12,36,60)")

p_25 <- ggplot(dtc,aes(x=StatedMonthlyIncomeRangeNumeric,y=LP_CustomerPayments_pc,color=factor(dtc$ProsperRating..numeric.))) + geom_line(stat='summary',fun.y=quantile,probs=.25) + scale_color_brewer(type="seq",palette="RdYlBu", guide = guide_legend(title='Rating', reverse = T, override.aes = list(alpha=1,size=2))) + coord_cartesian(xlim = c(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,probs = .99))) + scale_x_continuous(breaks=seq(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,.99),1000)) + facet_wrap(~Term) + ggtitle("25% quantile of Customer Payments as % of loan, facet by term (12,36,60)")

p_75 <- ggplot(dtc,aes(x=StatedMonthlyIncomeRangeNumeric,y=LP_CustomerPayments_pc,color=factor(dtc$ProsperRating..numeric.))) + geom_line(stat='summary',fun.y=quantile,probs=.75) + scale_color_brewer(type="seq",palette="RdYlBu", guide = guide_legend(title='Rating', reverse = T, override.aes = list(alpha=1,size=2))) + coord_cartesian(xlim = c(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,probs = .99))) + scale_x_continuous(breaks=seq(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,.99),1000)) + facet_wrap(~Term) + ggtitle("75% quantile of Customer Payments as % of loan, facet by term (12,36,60)")

grid.arrange(p_obs,p_75,p_median,p_25,ncol=1)
```

### Loan status with delinquencies
I want to look at in-progress loans, where loan status is delinquent.  I also want to look at how far along the loan is from the origination date.
...
?filter
table(filter(dt,LoanFirstDefaultedCycleNumber>0)$LoanStatus)
```
...
p1 <- ggplot(dtc,aes(x=LoanFirstDefaultedCycleNumber)) + geom_histogram(binwidth=1) + ggtitle("Number of borrowers for each month of first default")

p2 <- ggplot(dtc,aes(x=LoanFirstDefaultedCycleNumber, y=LP_CustomerPayments_pc, color=factor(dtc$ProsperRating..numeric.))) + geom_line(stat='summary', fun.y=median) + scale_color_brewer(type = 'seq', palette='RdYlBu',guide = guide_legend(title='Rating', reverse = T, override.aes = list(alpha=1,size=2))) + ggtitle("Customer payments ratio v. first default month")
...
There doesn't appear to be a relation between income and the first default cycle
```{r}
ggplot(dtc,aes(x=StatedMonthlyIncomeRangeNumeric,y=LoanFirstDefaultedCycleNumber, color=factor(dtc$ProsperRating..numeric.))) + geom_line(stat='summary',fun.y=mean) + coord_cartesian(xlim = c(0,quantile(dtc$StatedMonthlyIncomeRangeNumeric,probs = .99))) + scale_color_brewer(type = 'seq', palette='RdYlBu',guide = guide_legend(title='Rating', reverse = T, override.aes = list(alpha=1,size=2)))
```

### first defaulted cycle versus debt income ratio
Debt ratio doesn't show a relationship with how early a borrower defaults.
```{r}
ggplot(dtc,aes(x=DebtToIncomeRatio,y=LoanFirstDefaultedCycleNumber, color=factor(dtc$ProsperRating..numeric.))) + geom_line(stat='summary',fun.y=mean) + coord_cartesian(xlim = c(0,quantile(dtc$DebtToIncomeRatio,probs = .99,,na.rm=T))) + scale_color_brewer(type = 'seq', palette='RdYlBu',guide = guide_legend(title='Rating', reverse = T, override.aes = list(alpha=1,size=2)))
```

###  first default cycle vs. Loan original amount
```{r}
ggplot(dtc,aes(x=LoanOriginalAmount,y=LoanFirstDefaultedCycleNumber, color=factor(dtc$ProsperRating..numeric.))) + geom_point(stat='summary',fun.y=mean) + coord_cartesian(xlim = c(0,quantile(dtc$LoanOriginalAmount, probs=.90,,na.rm=T))) + scale_color_brewer(type = 'seq', palette='RdYlBu',guide = guide_legend(title='Rating', reverse = T, override.aes = list(alpha=1,size=2)))
```
...
```{r}
ggplot(dtc,aes(x=LoanOriginalAmount_per_Term_pc_Income,y=LoanFirstDefaultedCycleNumber, color=factor(dtc$ProsperRating..numeric.))) + geom_line(alpha=0.5, stat='summary',fun.y=median) + coord_cartesian(xlim = c(0,quantile(dtc$LoanOriginalAmount_per_Term_pc_Income, probs=.90,,na.rm=T))) + scale_color_brewer(type = 'seq', palette='RdYlBu',guide = guide_legend(title='Rating', reverse = T, override.aes = list(alpha=1,size=2)))
...
ggplot(dtc,aes(x=LoanOriginalAmount_per_Term_pc_IncomeRange,y=LoanFirstDefaultedCycleNumber, color=factor(dtc$ProsperRating..numeric.))) + geom_line(alpha=0.5, stat='summary',fun.y=median) + coord_cartesian(xlim = c(0,5)) + scale_color_brewer(type = 'seq', palette='RdYlBu',guide = guide_legend(title='Rating', reverse = T, override.aes = list(alpha=1,size=2)))

ggplot(dtc,aes(x=LoanOriginalAmount_per_Term_pc_IncomeRange,y=LoanFirstDefaultedCycleNumber)) + geom_line(alpha=0.5, stat='summary',fun.y=median) + coord_cartesian(xlim = c(0,5))
...
ggplot(dtc,aes(x=MonthlyLoanPayment,y=LoanFirstDefaultedCycleNumber)) + geom_line(alpha=0.5, stat='summary',fun.y=median) + coord_cartesian(xlim = c(0,5))
```
...
### Plot One
```{r Plot_One}
ggplot(dtc,aes(y=LP_CustomerPayments_pc,x=factor(ProsperRating..numeric.))) + geom_boxplot() + ggtitle("Customer payments v. rating for completed loans") + xlab("prosper rating (1 worst, 7 best)")
```

### Description One
When plotting cumulative customer payments for completed loans (those with a closed date), and separating by Prosper rating, we see a wider distribution for lower rated borrowers, and narrower distribution for higher rated borrowers.  Looking at the 25% and 75% quantiles, lower rated borrowers may pay more than higher rated borrowers, but they may also end up paying significantly less.

...
#Data overview
The data records represent each borrower.  It includes information related to their credit-worthiness at the time that they requested the loan.  It is also a snapshot of their cumulative payments at the time the data was retrieved.  Some records are of loans that are either completed or defaulted, and show the final result of the loan and payments.  Most others are current loans that are still in the process of being paid off.  The main interest is in viewing the relation bewteen information known at the time the loan was requested, and the final outcomes of loan payments.  What factors influence whether an investor is more likely to get paid in full?
...
# separate: drop the extra day, hour fields; convert char to int
...
```{r}
ggplot(data=dt,
       aes(x=factor(LoanOriginationYear))) + 
  geom_bar() + 
...
p2 <- ggplot(data = subset(dt,
                           dt$DebtToIncomeRatio > 0),
       aes(x = DebtToIncomeRatio)) + 
  geom_histogram(binwidth=.05) +
  coord_cartesian(xlim = c(.05,1.1)) +
  scale_x_log10(breaks = c(.01, .1, .2, .5, 1)) + 
  ggtitle("Debt income ratio log scale is more gaussian")
grid.arrange(p1,p2,ncol=1)
```
...
p2 <- ggplot(dt,
  ...
  scale_x_log10(breaks = c(500, 
                           ...
                           15000, 
                           20000)) + 
  coord_trans(limx = c(500,
                       20000)) + 
  ggtitle("Monthly income log scaled")
...
p3 <- ggplot(subset(dt,
  ...
  scale_x_log10(breaks = c(1,
                           ...
                           50)) + 
  scale_y_continuous(breaks = seq(0,5000,500)) + 
  ggtitle("Number of delinquencies log scaled")
...
p1 <- ggplot(dt,
  ...
  scale_x_continuous(breaks=c(seq(1000,10000,2000),
                              15000,
                              20000,
                              25000),
                     limits=c(1000,26000)) + 
...
p2 <- 
  ggplot(dt,
       aes(LoanOriginalAmount)) + 
  geom_histogram(binwidth=.05) + 
  scale_x_log10(breaks=c(seq(2000,10000,2000),
                              15000,
                              20000,
                              25000),
                     limits=c(1000,26000)) + 
  ggtitle("original loan amount log scaled")
...
ggplot(dt, 
       aes(factor(Term))) + 
  geom_bar() + 
    ggtitle("Most loans are for 36 months")
```
...
```{r}
# separate args: Convert char to numeric
dt <- tidyr::separate(data = dt,
                      col = ClosedDate, 
                      into = c("ClosedYear","ClosedMonth"),
                      sep = "-",
                      extra = "drop",
                      convert = T)
...
```{r}
ggplot(dt,
       aes(factor(ClosedYear))) + 
  geom_bar() + 
  ggtitle("Year when loan is closed (completed or defaulted/cancelled)")
```
...
Note that there is a negative net loss of -954, which might be a a data entry error.
```{r}
...
p2 <- ggplot(subset(dt,
       ...
       aes(LP_NetPrincipalLoss)) + 
  geom_histogram(binwidth = .05) + 
  scale_x_log10(breaks=c(500,1000,2500,4000,10000,20000),
                limits=c(500,25000)) +
  ggtitle("Log scale of net principal loss (units are not log)")
...
# Note that an equivalent subset is:
# dtc <- subset(dt,dt$LoanStatus %in% 
# c("Cancelled","Chargedoff","Completed","Defaulted"))
...
```{r}
"Customer Payments as fraction of loan"
...
```{r}
"Principal payments as fraction of loan"
...
"Interest and fees as fraction of loan"
...
p1 <- ggplot(dt, 
             aes(LoanFirstDefaultedCycleNumber)) +
  geom_histogram(binwidth = 1) + 
  ggtitle("First month of default")

p2 <- ggplot(dt, 
             aes(LoanFirstDefaultedCycleNumber)) +
  geom_histogram(binwidth = .05) + 
  scale_x_log10(breaks=c(2,5,10,15,20,30,40)) + 
  ggtitle("First month of default log scaled")

p3 <- ggplot(subset(dt,
                    dt$LoanCurrentDaysDelinquent > 0),
             aes(x = LoanCurrentDaysDelinquent)) + 
  geom_histogram() + 
  ggtitle("Days delinquent 
...
p4 <- ggplot(subset(dt,
  ...
  geom_histogram() + 
  ggtitle("Months since origination 
          for delinquent loans")
...
p2 <- ggplot(subset(dtc,
       ...
       aes(x=factor(ProsperRating..numeric.),
           y=LP_NonPrincipalRecoverypayments_pc)) + 
  geom_boxplot() + 
  ggtitle("borrowers who made recovery payments 
          paid more when they had lower ratings")
...
#?cut
# To avoid exponential notation, set dig.lab to 6, to show more digits
dtd <- transform(dtd,
...
dtd <- transform(dtd,
                 CreditScoreRangeLowerRange = 
                   cut(dtd$CreditScoreRangeLower,
                       breaks = quantile(dtd$CreditScoreRangeLower,
                                         seq(0,1,.1),
                                         na.rm=T), 
                       include.lowest = T))

#summary(dtd$CreditScoreRangeLowerRange)
...
p1 <- ggplot(dtd,
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  ggtitle("Defaults are more common among those that borrowed 5000 or less")

# Set dig.lab to 6 to avoid exponential notation (show more digits)
dtd <- transform(dtd,
...
#summary(dtd$LoanOriginalAmount_per_TermRange)
...
#summary(dtd$LoanOriginalAmount_per_Term_pc_IncomeRange)
  ...
  scale_color_brewer(type = "seq",
                     ...
                     guide = 
                       guide_legend(title = 'Credit Score', 
                                    ...
                                    override.aes = list(alpha = 1, 
                                                        size = 2))) + 
  xlab("Monthly Income in 10 quantiles") + 
  ylab("Customer Payments as fraction of original loan") + 
  coord_flip() +
  ggtitle("Higher incomes and higher ratings, 
          less risky payment outcomes") + 
...
grid.arrange(p_obs,
             ...
             p_75,
             ncol = 1,
             main = "Lower incomes, lower ratings, and 60 month term 
             have more under-payment risk")
...
grid.arrange(p1,
             p2,
             ncol = 1,
             main = "Earlier defaults are more common,
             and result in lower cumulative payments")
```

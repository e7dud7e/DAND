...
ggplot(dt,aes(StatedMonthlyIncome)) + 
  geom_histogram(binwidth=500) + 
  coord_trans(limx = c(0,quantile(dt$StatedMonthlyIncome,.99)))
```
...
ggplot(dt,aes(DelinquenciesLast7Years)) + geom_histogram(binwidth=1) + 
  coord_trans(limx = c(0,50), limy=c(0,5000)) + 
...
ggplot(dt,aes(LoanOriginalAmount)) + geom_histogram(binwidth=1000) + 
...
ggplot(dt,aes(Term)) + geom_histogram() + 
...
ggplot(subset(dt,dt$LP_NetPrincipalLoss > 0),aes(LP_NetPrincipalLoss)) + 
...
ggplot(dtd,aes(x=LP_CustomerPayments_pc)) + geom_histogram(binwidth=0.01) + 
  scale_x_continuous(breaks=seq(0,2,.1), lim=c(0,2))
```
...
ggplot(dtd,aes(x=LP_CustomerPrincipalPayments_pc)) + geom_histogram(binwidth=0.01) + 
  scale_x_continuous(breaks=seq(0,1.1,.1), lim=c(0,1.1))

summary(dtd$LP_InterestandFees_pc)
ggplot(dtd,aes(x=LP_InterestandFees_pc)) + geom_histogram(binwidth=0.01) + 
  scale_x_continuous(breaks=seq(0,1.1,.1), lim=c(0,1.1))
```
...
ggplot(dtc,aes(x=LP_CustomerPayments_pc,y=ListingNumber,color=ProsperRating..numeric.)) + 
  geom_point(alpha=0.1)

ggplot(dt,aes(y=LP_CustomerPayments_pc,x=factor(ProsperRating..numeric.))) + 
  geom_boxplot() + 
  ggtitle("Including in progress loans makes payments misleading")

ggplot(dtc,aes(y=LP_CustomerPayments_pc,x=factor(ProsperRating..numeric.))) + 
  geom_boxplot() + 
  ggtitle("Including only completed loans makes comparisons more clear")

ggplot(dtc,aes(y=LP_CustomerPayments_pc,x=factor(ProsperScore))) + 
  geom_boxplot() + 
  ggtitle("ProsperScore should only range from 1 to 10, why is there 11?")

ggplot(dtc,aes(x=LP_CustomerPayments_pc)) + 
  facet_wrap(~ProsperRating..numeric.,ncol=7) + 
  geom_histogram() + 
...
```{r}
ggplot(dtc,aes(y=LP_InterestandFees_pc,x=factor(ProsperRating..numeric.))) + 
  geom_boxplot() +
...
```{r}
ggplot(dtc,aes(y=LP_CustomerPrincipalPayments_pc,x=factor(ProsperRating..numeric.))) + 
  geom_boxplot() + 
...
ggplot(dtc,aes(y=LP_NonPrincipalRecoverypayments_pc,x=factor(ProsperRating..numeric.))) + 
  geom_boxplot() + coord_cartesian(ylim = c(0,0.1)) + 
...
ggplot(subset(dtc,dtc$LP_NonPrincipalRecoverypayments_pc>0),
       aes(y=LP_NonPrincipalRecoverypayments_pc,x=factor(ProsperRating..numeric.))) + 
  geom_boxplot() + 
...
```{r}
ggplot(dtc,aes(y=LP_GrossPrincipalLoss_pc,x=factor(ProsperRating..numeric.))) + 
  geom_boxplot() + 
...
```{r}
ggplot(dtc,aes(y=LP_NetPrincipalLoss_pc,x=factor(ProsperRating..numeric.))) + 
  geom_boxplot() + 
...
ggplot(dtd,aes(x=StatedMonthlyIncome,y=LP_CustomerPayments_pc)) + geom_point(alpha=.2) + coord_cartesian(xlim = c(0,quantile(dtd$StatedMonthlyIncome,probs = .99))) + ggtitle("Scatter plot of income vs. payments as % of loan")
...
ggplot(dtd,aes(x=StatedMonthlyIncomeRange,y=LP_CustomerPayments_pc)) + 
  geom_boxplot() + 
  coord_flip() + 
  ggtitle("Median payments are similar across income, with more variability in lower income")
...
ggplot(dtd,aes(x=DebtToIncomeRatio,y=LP_CustomerPayments_pc)) + 
  geom_point(alpha=0.1) + coord_cartesian(xlim = c(0,1.5)) + 
  ggtitle("Most debt income ratios are 0.5 or below; most customer payments are 100% or above")
...
dtd <- transform(dtd,DebtToIncomeRatioRange = 
...
ggplot(subset(dtd,!is.na(dtd$DebtToIncomeRatio)),
       aes(x=DebtToIncomeRatioRange,y=LP_CustomerPayments_pc)) + 
  geom_boxplot() + 
...
ggplot(dtd,aes(x=CreditScoreRangeLower,y=LP_CustomerPayments_pc)) + 
  geom_point(alpha=0.1,fill="grey",position="jitter") + 
  coord_cartesian(xlim=c(500,900),ylim=c(0,2)) + 
  scale_x_continuous(breaks=seq(500,900,25)) + 
  scale_y_continuous(breaks=seq(0,2,.1)) +
  ggtitle("Worse credit scores had wider range of payments; most paid at least 100% of loan")
...
ggplot(subset(dtd,!is.na(dtd$CreditScoreRangeLower)),
       aes(x=CreditScoreRangeLowerRange,y=LP_CustomerPayments_pc)) + 
  geom_boxplot() + 
  ggtitle("lower credit scores had slightly higher median payments, but more variance")
...
```{r}
#?geom_point
ggplot(dtd,aes(x=LoanOriginalAmount,y=LP_CustomerPayments_pc)) + 
  geom_jitter(alpha=0.01,color="navy") +
  coord_cartesian(xlim=c(0,20000),ylim=c(0,1.5)) + 
  scale_x_continuous(breaks=seq(0,20000,1000)) + 
  ggtitle("Defaults appear more common among those that borrowd 5000 or less")
...
ggplot(dtd,aes(x=LoanOriginalAmountRange,y=LP_CustomerPayments_pc)) + 
  geom_boxplot() + 
  coord_flip() + 
  ggtitle("Those that borrowed 10,000 or more, or between 3,000 to 4,000 had more underpayments")

ggplot(dtd,aes(x=StatedMonthlyIncomeRange,y=LoanOriginalAmount)) + 
  geom_boxplot() + 
  scale_y_continuous(breaks=seq(0,30000,1000)) +
  coord_flip() + 
  ggtitle("Borrowers with incomes under 6,800 per month borrowed $5,000 or less")

ggplot(subset(dtd,dtd$StatedMonthlyIncome <= quantile(dtd$StatedMonthlyIncome,.99)),
       aes(y=StatedMonthlyIncome,x=LoanOriginalAmountRange)) + 
  geom_boxplot() + 
  scale_y_continuous(breaks=seq(0,15000,1000)) + 
  coord_flip() + 
  ggtitle("The lower 70% of loans ($7,450 or less), had incomes at $6,000 or less")
...
dtd <- transform(dtd, LoanOriginalAmount_per_TermRange = 
                   cut(dtd$LoanOriginalAmount_per_Term, 
                       breaks = quantile(dtd$LoanOriginalAmount_per_Term, 
                                         probs = seq(0,1,.1), na.rm = T),
                       include.lowest = T))

ggplot(dtd,aes(x=LoanOriginalAmount_per_TermRange,y=LP_CustomerPayments_pc)) + 
  geom_boxplot() +
  ggtitle("90% of loans were about $360 per month or less (ignoring interest)")

ggplot(dtd,aes(x=factor(Term),y=LoanOriginalAmount)) + geom_boxplot() +
  ggtitle("60 month terms tended to be for larger loans, around $10,000")
```
...
ggplot(dtc,aes(y=CreditScoreRangeLower,x=ProsperRating..numeric.)) + geom_point(alpha=0.5, stat='summary',fun.y=median) + ggtitle("Lower credit scores have lower Prosper ratings")

ggplot(dt,aes(y=CreditScoreRangeLower,x=factor(ProsperRating..numeric.))) + geom_boxplot() + ggtitle("Lower credit scores have lower Prosper ratings")
```
...
```{r}
ggplot(dt,aes(x=CreditScoreRangeLower,y=LoanFirstDefaultedCycleNumber)) + geom_line(alpha=0.5, stat='summary', fun.y=median) + ggtitle("Lower credit scores tended to default earlier")
```
...
I found some indication that better credit ratings, higher incomes, and 3-year terms showed better payment outcomes.  
